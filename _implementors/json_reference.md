...
| `features` (proposed) | object | An object of dev container features and related options to be added into your primary container. The specific options  that are available varies by feature, so see [its documentation](../features#a-hrefdevcontainer-json-properties-namedevcontainer-json-properties-classanchor-devcontainerjson-properties-a) for additional details. For example: <br />`"features": {"github-cli": "latest"}` <br /><br /> ⚠️ Support is currently in preview as the [spec](../features) is not yet final. |
...
| `customizations` | object | Product specific properties, defined in [supporting tools](../../supporting) |
{: .table .table-bordered .table-responsive}
